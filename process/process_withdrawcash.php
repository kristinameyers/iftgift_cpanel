...
if ($_GET['wid'])
{ 
	$catid=addslashes($_GET['wid']); 
	$wstatus=addslashes($_GET['wstatus']); 
	$notes=addslashes($_GET['note']);
	...
	if($wstatus=='approved'){
	$CusInfo = mysql_fetch_array(mysql_query("select email,first_name,last_name from ".tbl_user." where userId = '".$OrderInfo['userId']."'"));
    				...
    				<div style="height:364px;font-size:15px !important; line-height:0.95">
    					...
    					<div style="padding-left: 13px; padding-top: 15px;">
      						<div style="-moz-border-radius:6px;-webkit-border-radius:6px;background: #fff;margin-top: 20px;border: 1px solid #cccccc;width: 602px;padding-top: 15px;height: 232px;" >
								<p style="margin-left:10px;"><strong>Dear</strong> : '.$CusInfo['first_name'].' '.$CusInfo['last_name'].'</p>
	...
	/*******************************************START SEND MAIL OF WITHDRAW CASH PROCESS [For User]*****************************************************************/
			...
			//$mailsent = mail($to,$subject,$message,$headers);	
			sendmail_admin($to,  $subject, $message,$headers);
			$cash=mysql_fetch_array(mysql_query("SELECT netamount,withdrawID from gift_cash_withdraw where withdrawID = '".$catid."'")); 
			$cur_cash=$cash['netamount'];
			$netamount = str_replace('$','',$cur_cash);
			$user_cash =mysql_fetch_array(mysql_query("SELECT available_cash,userId FROM ".tbl_user."  where userId='".$OrderInfo['userId']."'"));
			$net_cash=$user_cash['available_cash'];
			$tot_cash=$net_cash + $netamount;
				if($user_cash){  
					$del_payment=$db->query("UPDATE ".tbl_user."  set available_cash='".$tot_cash."'  where userId  = '".$OrderInfo['userId']."' "); 
					if($tot_cash){
						echo $tot_cash;
					}
			} 	
