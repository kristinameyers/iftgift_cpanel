...
foreach($_POST['col'] as $k=> $v)
	...
	$PRefix_query = '';
		...
		if(count($dups)>0){
			...
			$query=mysql_query("SELECT pro_name FROM ".tbl_product."  WHERE pro_name ='".mysql_real_escape_string(stripslashes(trim($row[2])))."'");
			 $count_name=mysql_fetch_array($query); 
			 $pr_name=$count_name['pro_name']; 
			if($pr_name)
			{
				$_SESSION['import_errs']['msg'] = "Record Already Exists";	
				header('location:'.ruadmin.'home.php?p=importer'); exit;
			} else{
