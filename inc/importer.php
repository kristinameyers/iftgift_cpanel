...
foreach($_POST['col'] as $k=> $v)
	...
	$PRefix_query = '';
		...
		if(count($dups)>0){
			...
			$query =mysql_query("SELECT * FROM ".tbl_product."  WHERE pro_name ='".$row[2]."'");
			if(mysql_num_rows($query) > 0){ //echo "there"; exit;
			while($rs = mysql_fetch_array($query)){
				$pr_name=$rs['pro_name']; 
				$proid=$rs['proid'];
			 $query =mysql_query("UPDATE ".tbl_product." set image_code='".$row[0]."', vendor='".mysql_real_escape_string(stripslashes(trim($row[1])))."',pro_name='".mysql_real_escape_string(stripslashes(trim($row[2])))."',price='".mysql_real_escape_string(stripslashes(trim($row[3])))."',category='".$pcategory."',sub_category='".$subcategory."',gender='".preg_replace('/\s+/', '',str_replace(' ', '',mysql_real_escape_string(stripslashes(trim($row[5])))))."' WHERE proid='".$rs['proid']."'"); 
			 
				$_SESSION['import_errs']['msg'] = "Record Updated  successfully ";	
			} }  else { 
